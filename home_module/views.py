...
class HomeView(View):
    def get(self, request: HttpRequest) -> HttpResponse:
        ...
        categories: Category = (
            ...
            .annotate(count_products=Count("products_category"))
            .order_by("?")
        )
        products_discount: Product = Product.objects.filter(
            is_active=True, product_variant__discount__isnull=False
        ).annotate(
            discount=Max("product_variant__discount"), rating=Avg("comments__rating")
        )[:6]
        old_time = timezone.now() - timedelta(days=30)
        products_new: Product = (
            Product.objects.filter(is_active=True, created_at__gte=old_time)
            .annotate(
        ...
        variant_sales = (
            ...
            .values("product")
            .annotate(total_sales=Sum("sales_count"))
            .values("total_sales")
        )
        products_sales_week = (
            Product.objects.filter(is_active=True)
            .annotate(
                sales_count=Subquery(variant_sales[:1]),
                discount=Max("product_variant__discount"),
                rating=Avg("comments__rating"),
            )
        ...
        high_rating_products = (
            base_qs.annotate(
                rating=Avg("comments__rating"),
                max_discount=Max("product_variant__discount"),
            )
...
def header_component(request: HttpRequest) -> HttpResponse:
    ...
    if request.user.is_authenticated:
        try:
            ...
            count_order_item = Order.objects.filter(
                user_id=user_id, status="cart", is_active=True
            ).aggregate(Count("order_items"))["order_items__count"]
