...
class RegisterView(View):
    ...
    def post(self, request: HttpRequest) -> HttpResponseRedirect:
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "حساب کاربری شما با موفقیت ساخته شد و ایمیلی جهت فعال شدن اکانت شما ارسال گردید",
            )
            ...
            send_email(
                ...
                "account_module/emails/active_account.html",
            )
            return redirect(reverse("account_module:login_page"))
        else:
            messages.error(
                request,
                "کاربری با این مشخصات وجود دارد یا کپتجا به درستی وارد نشده است",
            )
            return redirect(reverse("account_module:register_page"))


def active_account(
    request: HttpRequest, email_active_code: str
) -> HttpResponseRedirect:
    try:
        current_user: User = User.objects.get(
            email_active_code__exact=email_active_code, is_active=False
        )
        ...
        if not request.user.is_authenticated:
            ...
            if login_form.is_valid():
                try:
                    ...
                    messages.error(
                        request, "مشخصات وارد شده اشتباه یا اکانت شما فعال نیست"
                    )
                    return redirect(reverse("account_module:login_page"))
            else:
...
class EditProfileView(View):
    ...
    def post(self, request: HttpRequest) -> HttpResponseRedirect:
        user: User = request.user
        form: EditProfileFormModel = EditProfileFormModel(
        ...
        if form.is_valid():
            ...
            if current_user.email == email:
                ...
                if dip_user is not None:
                    messages.error(
                        request,
                        "کاربری با این ایمیل وجود دارد ایمیل دیگری را انتخاب فرمایید",
                    )
    ...
    def get(self, request) -> HttpResponse | HttpResponseRedirect:
        if request.user.is_authenticated:
            try:
                ...
                messages.error(
                    request, "چنین کاربری پیدا نشد که ادامه کار را انجام دهیم"
                )
                return redirect(reverse("account_module:login_page"))
        else:
            messages.error(
                request, "برای تغییر پسوورد خود ابتدا باید وارد حساب کاربری خود شوید"
            )
            return redirect(reverse("account_module:login_page"))
        ...
        try:
            ...
            if current_user is not None:
                send_email(
                    "بازیابی کلمه عبور",
                    current_user.email,
                    {"email_active_code": current_user.email_active_code},
                    "account_module/emails/reset_password.html",
                )
                return render(request, "account_module/forget_password_success.html")
        except User.DoesNotExist:
            messages.error(
                request, "کاربری با این مشخصات وجود ندارد یا حساب کاربری فعال نیست"
            )
...
class ResetPasswordView(View):
    def get(
        self, request: HttpRequest, email_active_code: str
    ) -> HttpResponse | HttpResponseRedirect:
        try:
            current_user: User = User.objects.get(
                email_active_code__exact=email_active_code, is_active=True
            )
            context = {
    ...
    def post(
        ...
        try:
            ...
            if password == confirm_password:
                ...
                return redirect(
                    reverse(
                        "account_module:reset_password_page",
                        kwargs={"email_active_code": current_user.email_active_code},
                    )
                )
        except User.DoesNotExist:
            messages.error(request, "کاربری با این مشخصات وجود ندارد که رمزش عوض بشه")
