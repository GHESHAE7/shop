...
class ProductsListView(ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        query = (
            ...
            .annotate(
                discount=Max("product_variant__discount"),
                rating=Avg("comments__rating"),
                sales_count=Sum("product_variant__sales_count"),
            )
            .order_by("-created_at")
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["max_price"] = (
            Product.objects.filter(is_active=True).aggregate(Max("price"))["price__max"]
            or 0
        )
        context["min_price"] = (
            Product.objects.filter(is_active=True).aggregate(Min("price"))["price__min"]
            or 0
        )

        if self.request.build_absolute_uri() == self.request.build_absolute_uri(
            reverse("product_module:products_page")
        ):
            context["title_heading"] = "محصولات"
            context["title"] = "تمام محصولات فروشگاه"
            context["show_discount"] = True

        elif self.request.build_absolute_uri() == self.request.build_absolute_uri(
            reverse("product_module:products_discount_page")
        ):
            ...
            reverse(
                ...
                kwargs={"category_url": self.kwargs.get("category_url")},
            )
        ):
            ...
            reverse(
                "product_module:products_by_brand_page",
                kwargs={"brand_url": self.kwargs.get("brand_url")},
            )
        ):
...
class ProductDetailView(DetailView):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        query = (
            ...
            .annotate(discount=Max("product_variant__discount"))
        )
        return query

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        attributes = ProductVariant.objects.filter(
            is_active=True, product=self.object
        ).order_by("size")
        ...
        context["stock"] = (
            Product.objects.filter(id=self.object.id, is_active=True).aggregate(
                Sum("product_variant__stock")
            )["product_variant__stock__sum"]
            or 0
        )
        context["count_comments"] = (
            Comment.objects.filter(is_active=True, product=self.object).aggregate(
                Count("id")
            )["id__count"]
            or 0
        )
        context["rating"] = (
            f"{Comment.objects.filter(is_active=True, product=self.object).aggregate(Avg('rating'))['rating__avg'] or 0:.2f}"
        )
        return context
...
def stock_color_size(request: HttpRequest) -> JsonResponse:
    try:
        ...
        get_product_variant: ProductVariant = ProductVariant.objects.values(
            "stock", "discount", "product__price"
        ).get(
            is_active=True, color__exact=color, size__exact=size, product_id=product_id
        )
        return JsonResponse(
            {
                "color": color,
                ...
                "price": get_product_variant["product__price"]
                - (
                    (get_product_variant["product__price"] / 100)
                    * get_product_variant["discount"]
                )
                if get_product_variant["discount"]
                else get_product_variant["product__price"],
            }
        )
    except ProductVariant.DoesNotExist:
        return JsonResponse({"message": "چنین محصولی وجود ندارد"})
